        ...
        if (stack != null) {
            ...
            if (ReflectorForge.isItemDamaged(stack)) {
                if (ModuleManager.getMod(BetterHotbar.class).getStage()) {
                    int j1 = (int) Math.round(16.0D - (double) stack.getItemDamage() * 16.0D / (double) stack.getMaxDamage());

                    RoundedUtil.drawRound(xPosition,yPosition+18,16,2,1,new Color(255,255,255,100));
                    RoundedUtil.drawRound(xPosition,yPosition+18,j1,2,1,new Color(255,255,255,200));
